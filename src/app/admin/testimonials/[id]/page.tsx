...
export default function EditTestimonial({
  ...
  const loadPageBasedImages = useCallback(
    async (pageNumber: number, pageRange?: { start: number; end: number }) => {
      try {
        ...
        for (const page of pagesToLoad) {
          ...
          if (response.ok) {
            ...
            const pageBased = data.images.filter(
              (img: TestimonyImage) => img.isPageBased
            );
  ...
  const addTag = (tag: string) => {
    if (!testimony || !tag.trim() || testimony.tags.includes(tag.trim()))
      return;
  ...
  const removeTag = (tagToRemove: string) => {
    ...
    updateField(
      "tags",
      testimony.tags.filter((tag) => tag !== tagToRemove)
    );
  };
  ...
  const removeImage = (imagePath: string) => {
    ...
    updateField(
      "images",
      (testimony.images || []).filter((img) => img !== imagePath)
    );
  };
  ...
  }, [id, router, loadMetadata, loadTestimony]);
        ...
        <div className="bg-white shadow rounded-lg p-6 mb-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              {!isNew && <p className="text-gray-600">ID: {testimony.id}</p>}
            </div>
            <button
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField("page", parseInt(e.target.value) || 0)
                  }
                  required
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mb-2">
                If this testimonial spans multiple pages, specify the range to
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-4 text-xs">
                  ...
                  <div className="flex items-center gap-1">
                    <span className="inline-flex items-center px-1 py-0.5 text-xs font-medium rounded bg-green-100 text-green-800">
                      Manual
                    </span>
                        ...
                        >
                          ...
                          <div className="p-2 bg-white">
                            <div className="text-xs text-black truncate mb-1">
                              {isPageBased
                                ? pageBasedImage.sectionTitle
                                : imagePath.split("/").pop()}
                            </div>
                            {isPageBased && pageBasedImage.photoNumber && (
                              <div className="text-xs text-gray-500 mb-1">
                                Photo #{pageBasedImage.photoNumber}
                              </div>
                            )}
                            <div className="mt-1">
                              <input
                                type="text"
                                value={
                                  testimony.imagesCaptions?.[imagePath] || ""
                                }
                                onChange={(e) =>
                                  updateImageCaption(imagePath, e.target.value)
                                }
            ...
            {pageBasedImages.length > 0 && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Page-Based Images for{" "}
                  {testimony.pageRange &&
                  testimony.pageRange.start !== testimony.pageRange.end
                    ? `Pages ${testimony.pageRange.start}-${testimony.pageRange.end}`
                    : `Page ${testimony.page}`}
                </label>
                <p className="text-sm text-gray-500 mb-4">
                  These images follow the naming convention and are
                  automatically associated with this page. You can link or
                  unlink them from this testimonial.
                </p>
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={() =>
                            isLinked
                              ? unlinkPageBasedImage(image.path)
                              : linkPageBasedImage(image.path)
                          }
